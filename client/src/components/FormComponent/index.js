...
export default class FormComponent extends React.Component {
  ...
  componentDidMount() {
    ...
    fetch('api/memes')
      .then(res => res.json())
      .then((data) => {
        initialMemes = data["Memes"]["Videos"].map((meme) => { return meme.name });
              ...
              <Card.Body>
                <Form>
                  <Form.Group as={Row} controlId="formHorizontalEmail">
                    ...
                    <Col sm={10}>
                      <select value={selectedMethod } onChange={e => this.setState({ selectedMethod: e.target.value })} className={"custom-select"}>
                        <option selected disabled> Select a method for displaying memes. </option>
                  ...
                  {(selectedMethod === "selected") && (
                    ...
                    <Col sm={10}>
                      <select value={selectedMeme} onChange={e => this.setState({ selectedMeme: e.target.value })} className={"custom-select"}>
                        <option selected disabled> Select a meme to display. </option>
                        { memes.map((meme) => { return <option key={meme}> {meme} </option> })}
                      </select>
                    </Col>
                  </Form.Group>)}
              ...
              <Card.Body>
                <Form>
                  <Form.Group as={Row} controlId="formHorizontalEmail">
                    ...
                    <Col sm={10}>
                      <input type={"text"} className={"form-control"} onChange={e => this.setState({ uploadMemeName: e.target.value })} placeholder={"Enter a name for the meme."} />
                    </Col>
                  </Form.Group>
                  <br/>
                  <Form.Group as={Row} controlId="formHorizontalEmail">
                    ...
                    <Col sm={10}>
                      <input type={"text"} className={"form-control"} onChange={e => this.setState({ uploadMemeURL: e.target.value })} placeholder={"Enter the URL for the meme."} />
                    </Col>
                  </Form.Group>
                </Form>
                <br/>
                {(uploadMemeName && uploadMemeURL) && (<Button onClick={this.handleUploadSubmit} variant={"success"} block> Upload </Button>)}
              </Card.Body>
            </Accordion.Collapse>
          </Card>
              ...
              <Card.Body>
                <Form>
                  <Form.Group as={Row} controlId="formHorizontalEmail">
                    <Form.Label column sm={2}> Meme Selector: </Form.Label>
                    <Col sm={10}>
                      <select defaultValue={1} value={selectedMemeToRemove} onChange={e => this.setState({ selectedMemeToRemove: e.target.value })} className={"custom-select"}>
                ...
                <br/>
                {(selectedMemeToRemove) && (<Button onClick={this.handleRemoveEntry} variant={"success"} block> Remove Entry </Button>)}
                <br/>
              </Card.Body>
            </Accordion.Collapse>
          </Card>
